...
use \PropelPDO;
use GEA\SoporteBundle\Model\Menu;
use GEA\SoporteBundle\Model\Perfil;
use GEA\SoporteBundle\Model\PerfilMenu;
use GEA\SoporteBundle\Model\PerfilMenuPeer;
...
abstract class BasePerfilMenuQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'GEA\\SoporteBundle\\Model\\PerfilMenu';
        }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \GEA\SoporteBundle\Model\PerfilQuery A secondary query class using the current class as primary query
     */
    public function usePerfilQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPerfil($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Perfil', '\GEA\SoporteBundle\Model\PerfilQuery');
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \GEA\SoporteBundle\Model\MenuQuery A secondary query class using the current class as primary query
     */
    public function useMenuQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinMenu($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Menu', '\GEA\SoporteBundle\Model\MenuQuery');
    }
